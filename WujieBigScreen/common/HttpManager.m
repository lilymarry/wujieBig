            ...
            andFail:(HttpFailureBlock)failure
{
    ...
    BaseAFNetworkingManager *baseAFNetworkingManager = [BaseAFNetworkingManager defaultManager];
    [baseAFNetworkingManager sendRequestMethod:HTTPMethodDELETE url:baseurl apiPath:url parameters:params progress:nil success:^(BOOL isSuccess, id  _Nullable responseObject) {
        if (success == nil) return ;
        success(responseObject);
    } failure:^(NSError * _Nullable error) {
        if (failure == nil) return ;
...
//+ (void)postShopUploadSingleImageWithUrl:(NSString *)url  baseurl:(NSString *)baseurl andImageName:(UIImage *)image andKeyName:(NSString *)keyName andParameters:(NSDictionary *)params andSuccess:(HttpSuccessBlock)success andFail:(HttpFailureBlock)failure
//{
//    if (url == nil) {
//        url = @"";
//    }
